...
1. **对话主模型（2026-06-14）** — [STALE] — References a date older than 30 days and is not the current main model.
2. **Issue 处理规则** — [CURRENT] — No explicit date, and the workflow is still relevant.
3. **测试质量标准（Issue #9 经验）** — [CURRENT] — No explicit date, and the standards are timeless.
4. **飞书 Bot 互通规则** — [STALE] — Contains a tester secret marked invalid after 2026-06-16.
...
- The DeepSeek model from Jun 14 is still [CURRENT] as it is the current main model.
- The stale tester secret (cli_aabbef59fcb9dcc7) is [STALE] and no longer actionable.
- The cross-profile Hindsight config section does not belong here and is [STALE].
...
## Historical
- 对话主模型（2026-06-14）
  - deepseek-v4-flash（provider=deepseek），保留 GLM_API_KEY 备用
```

This draft consolidates the MEMORY.md content by removing redundancy and moving stale items to the "## Historical" section. It also removes items that belong to other profiles.