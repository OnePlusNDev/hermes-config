...
- **Profiles can be HYBRID — flat-file AND hindsight bank coexist.** `MEMORY.md`/`USER.md` live on disk AND a hindsight bank (`bank_id: demo-tester`) can exist simultaneously with its own facts (30 nodes, 254 links). Maintenance must check BOTH: file mtime/inline dates for the flat files, and the bank's `/stats`/`/memories/list` for bank content. Don't assume one or the other. If only one exists, flag it explicitly in the report.
- **Archive naming with hardcoded dates causes overwrite.** Scripts like `archive_and_consolidate.py` that write to `archive/MEMORY-20260617.md` with a fixed date string will overwrite the same file on every run, losing historical snapshots. Always use `datetime.now().strftime("%Y%m%d")` for timestamped archive filenames.
