...
```
fatal: unable to access 'https://github.com/OWNER/REPO.git/': Error in the HTTP2 framing layer
```

The network *is* reachable — `curl https://github.com` returns HTTP 200 in under 2 s — but git's HTTP/2 stack can't negotiate properly.

**Diagnostic:** compare curl vs git:
```bash
# This succeeds → network is up
curl -s -o /dev/null -w "HTTP %{http_code} (%{time_total}s)\n" https://github.com

# This fails → HTTP/2 transport is broken
git ls-remote https://github.com/OWNER/REPO.git 2>&1 | head -3
```

**Fix — force git to use HTTP/1.1:**
```bash
# Per-repo (preferred, avoids affecting other repos)
...
This can also be combined with `gh auth git-credential` when the credential helper is also needed:
```bash
git config --local credential.helper '!gh auth git-credential'
git config --local http.version HTTP/1.1
git push origin main
