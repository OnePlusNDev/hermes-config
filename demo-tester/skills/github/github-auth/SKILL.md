...
### One-Off Credential Helper (No Config Change)

For a single push without modifying `git config`, use the `-c` flag — useful in cron jobs where you don't want to mutate the profile's git configuration:

```bash
git -c credential.helper='!gh auth git-credential' push origin main
```

This is also the preferred pattern when raw `git push` to `github.com:443` times out (HTTP/2 framing stall) because `gh auth git-credential` authenticates through `gh`'s Go HTTP client, which can handle connections that git's libcurl transport cannot. See the `github-repo-management` skill's reference `cron-mode-config-backup-workflow.md` for the full diagnostic flow and combined flags (`-c http.version=HTTP/1.1`, `--force`).
