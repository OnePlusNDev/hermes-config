...
| Profile backup | `references/hermes-profile-github-backup.md` (REST API → .env based; fallback when gh CLI unavailable) + `references/hermes-profile-rsync-github-backup.md` (rsync + git push; preferred when `gh auth status` shows logged-in with repo scope) |
