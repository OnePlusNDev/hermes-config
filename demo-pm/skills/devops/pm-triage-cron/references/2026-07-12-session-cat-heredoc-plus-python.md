...
**为什么 `***` 仍然能工作？** 虽然 `read_file` 查看脚本内容时显示 `Authorization: Bearer ***`（脱敏），但 `.env` 文件中实际存储的是完整 token（已验证：xxd 十六进制为 `[GHP_REDACTED]`）。`source .env` 在运行时加载真实值到 `$GITHUB_TOKEN` 变量，然后 curl 用 `$GITHUB_TOKEN` 引用它。
...
| `.env` 实际存储完整 token（xxd 确认） | ✅ `ghp_Z1...bghiu` |
| 空结果声明 `[SILENT]` | ✅ 抑制通知发送 |
