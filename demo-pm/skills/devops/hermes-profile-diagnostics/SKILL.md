...
- **`ps aux | grep hindsight-api` can miss a LIVE daemon entirely — always cross-check with `lsof`.** Verified 2026-09-19 (demo-pm): `ps aux | grep hindsight-api | grep -v grep` returned **empty** while the global `:8888` daemon was up and answering `{"status":"healthy"}` → it appeared only as a `python3.1` process (`lsof -nP -iTCP -sTCP:LISTEN` showed `python3.1 <pid> ... TCP 127.0.0.1:8888 (LISTEN)`). The global/standalone daemon is not necessarily launched through the `hindsight-api` entrypoint, so process-name grep under-reports. Use `lsof -nP -iTCP -sTCP:LISTEN | grep -E '9177|9178|9179|9180|8888'` as the ground-truth port scan **before** concluding "all daemons are down" or triggering a restart — otherwise you can restart a profile daemon that was never the one serving traffic, or misdiagnose sibling availability.
...
- **Reflect/consolidate LLM call can crash the daemon.** When hindsight encounters an LLM provider error (timeout, auth failure, transient API error) during reflect or consolidate, the daemon may exit abruptly (process disappears, `lsof -i :<port>` returns nothing, curl gets connection refused) instead of returning an error response. Recovery: find the daemon's `--port` and restart it (see Step B), verifying with `curl health`. The crash is often transient — retrying the reflect call with `--budget low` after restart usually succeeds.
...
- **Hindsight daemon restart latency is variable: ~15s when PG is warm, 1-3+ min when cold.** The 1-3+ minute figure only applies when the embedded PostgreSQL must initialize from scratch. If pg0 PostgreSQL is ALREADY running (`ps aux | grep postgres` — any sibling or global :8888 daemon on the same machine shares `~/.pg0`), the profile daemon becomes healthy in ~15 seconds (observed 2026-08-08: poll health every 5s, healthy on 3rd poll). Check PG state first and poll with short intervals; only assume the long wait when no postgres process exists. The API is unreachable during the startup window (`curl health` returns connection refused) either way. If a cron job needs the daemon and it's down, proxy through a sibling daemon (see `references/memory-maintenance.md`) instead of waiting for restart.
- **Distinguishing "daemon still starting" vs "daemon crashed":** Both show connection refused on `curl health`. Check `lsof -i :<port>` — if a process listens on the port but health returns non-200, it's still starting (wait longer). If `lsof` shows nothing listening AND `ps aux | grep hindsight-api` shows no matching PID, the daemon crashed. Recovery requires restart, not patience.
...
- **HuggingFace SSL/timeout blocks daemon start even with cached models.** The daemon verifies embedding model freshness against huggingface.co at startup. If HF is unreachable, two failure modes: (a) `RuntimeError: Model/connection initialization did not complete within 300s` (download timeout), or (b) `[Errno 61] Connection refused` on HEAD request → `Cannot send a request, as the client has been closed.` (network unreachable). Fix: set `HF_ENDPOINT=https://hf-mirror.com` before starting. **Simpler:** set `HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1` to skip all network verification when models are cached. After a failed start, **cleanup leftover `pg0` postgres processes** before retrying — they persist after the daemon crashes (`ps aux | grep pg0 | awk '{print $2}' | xargs kill`). See `references/2026-07-26-daemon-hf-connection-refused-postgres-cleanup.md`.
