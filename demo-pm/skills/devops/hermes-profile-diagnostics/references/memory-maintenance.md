...
**Batch the independent reads — a whole clean run fits in ~8 tool calls (verified 2026-09-25).** Steps 1–3 are mutually independent (file ages, char counts, memory-limit grep, `lsof` port scan, sibling `/v1/default/banks`, >90d session count, retention config) → put them in **one** `terminal()` call with several `echo` sections. Steps 5–7 likewise (op-status poll + bank stats + post-run session count) → one call. Only the daemon start (background + health poll loop) and the reflect POST need their own round-trips. Do not spend one call per check — the flow above is a *sequence*, not a call-per-step budget.
...
**⚠️ Correction (2026-09-17) — token count is NECESSARY but NOT SUFFICIENT. The two gates are independent; always apply both.** On 2026-09-17 the `budget=high` HTTP primary reflect returned the **bare-affirmation false positive** 「存在需要归档的过期事实（30天以上）。」 at **6,824 input tokens — i.e. AT/above the ~6.8K baseline**, so the token gate alone would have waved it through as trustworthy. The wording gate caught it (`裸断言` = a restatement of the question's own options, no analysis), the standing targeted disambiguation reflect then returned 「无矛盾、无冗余。」 (6,967 tokens). **Do not treat "tokens ≥ baseline" as a clean bill of health.** Decision procedure, in order:
1. Read the wording first — if the answer merely restates the question's options (「存在需要归档的过期事实」/「存在重复信息」) with no analysis of *which* items, it is the false positive **regardless of token count** → run the disambiguation reflect.
2. Read `input_tokens` second — far below baseline (2–5K) means facts were never fed → per the 09-15 correction go to HTTP `budget: "high"` (not the disambiguation reflect, which degrades and hallucinates at that point).
3. Clean **conclusion** (「无需要归档的过期事实、重复信息或矛盾信息」/「无矛盾、无冗余」) **AND** tokens ≥ baseline = actually clean.
The 09-16 healthy run happened to satisfy both; the 09-17 run satisfied only the token gate. Since the false-positive branch recurs intermittently, budget for the disambiguation reflect as a routine second step rather than a rare rescue.
...
- `hindsight_client.reflect(..., budget='low', include_facts=True)` is no longer the default of record: **prefer HTTP `budget: "high"`** as the primary reflect for the demo-pm bank (verified clean 2026-09-15, 6,827 tokens / 18s). **Re-verified 2026-09-16: clean conclusion on the FIRST attempt, 6,821 input tokens / ~43s, no disambiguation reflect needed** — the budget=high-first rule holds after the 09-15 degradation episode. **Also re-confirmed 2026-09-16: sibling daemons being healthy is NOT a substitute** — :9177 and :8888 both answered `/health` but their bank lists held only `active`/`demo-dev`/`hermes`, so the profile's own :9178 daemon had to be started before any bank operation.
...
**⚠️ A second, usually-stale mirror of ARCHIVE.md exists under the config repo — ignore it.** Verified 2026-09-23 (demo-pm): `~/.hermes/repos/hermes-config/<profile>/memories/archive/ARCHIVE.md` (44 lines, newest log row **2026-07-24**) vs the profile copy at `~/.hermes/profiles/<profile>/memories/archive/ARCHIVE.md` (71 lines through 2026-09-22). The **profile copy is authoritative** for this cron — append only there. Do NOT spend the run diffing or syncing the repo mirror; it is presumably written by a separate backup job on its own schedule. If the gap looks significant, mention it as a low-priority note in the report (that is all the user needs — a bare observation, not a sync attempt).
...
**Pitfall — Sibling/global daemon healthy ≠ it serves YOUR bank.** `/health` only proves a daemon is up; it says nothing about which database it is attached to. Observed 2026-08-03: global :8888 returned `{"status":"healthy"}` but its bank list showed `demo-dev` created 2026-08-01 with `fact_count` 0, while the profile daemon :9178 showed the SAME bank_id `demo-dev` created 2026-07-25 with 26 facts plus `demo-pm-memory` (48 facts). Same bank_id, different databases — proxying through the healthy sibling would have served empty/wrong data. **Before relying on sibling fallback, check the sibling's bank list (`curl -s http://127.0.0.1:<port>/v1/default/banks`) and confirm YOUR bank_id appears with the expected fact_count/created_at.** If it doesn't match, start the profile's own daemon instead (see hindsight-daemon-recovery skill).
...
**Pitfall — `operations_by_status.completed` can lag a run; trust the op's own status, not the counter.** Verified 2026-09-19 (demo-pm): stats read `operations_by_status: {"completed": 49}` **both before and after** a consolidation op that returned `status: "completed"` — the counter did not increment (prior days showed the expected `n → n+1`, e.g. 47→48, 48→49). The authoritative signals are `GET /v1/default/banks/<bank>/operations/<op_id>` → `status: "completed"`, plus `pending_operations: 0` / `failed_operations: 0`. Do NOT re-run consolidation or record a failure just because the counter didn't move. **Both behaviors are normal — the counter may also increment cleanly (observed 2026-09-23: `53 → 54` for a completed op; 09-22: stayed `52 → 52`).** **Also observed 2026-09-25: a single completed op moved it `54 → 56` (+2).** So the delta has at least three normal shapes — `+0`, `+1`, `+2`. Do NOT infer from a multi-step jump that two operations ran, that a previous op was lost, or that consolidation double-fired; `+2` is just another noisy shape. Treat the counter as uninformative in every direction and keep the op's own `status` plus `pending_operations: 0` / `failed_operations: 0` as the sole verdict.
