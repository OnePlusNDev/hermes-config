...
- **gh CLI 的 active 账号会漂移，每轮先查再动手（2026-09-23 更正）：** keyring 里登录了多个账号（`OnePlusNTester` / `OnePlusNDev` / `JungleAssistant` / `OnePlusNPM` / …）。曾经的记录「active 账号是 OnePlusNDev」**已不成立**——2026-09-17 与 2026-09-23 两轮实测 `gh api user --jq '.login'` 都返回 **`OnePlusNTester`**（非仓库属主、无 push 权限），必须 `gh auth switch --user OnePlusNDev` 切回属主后再操作 hermes-config。**不要凭记忆假设当前是谁**：每轮开头跑一次 `gh api user --jq '.login'`，不等于 `OnePlusNDev` 就切；`gh auth status` 可一次列出全部已登录账号（含 `Active account: true` 标记）。
...
- **⚠️ 兄弟 profile 不只是覆盖，还会「整片清扫」共享 /tmp 命名空间（2026-09-23 实证，导致真实故障）：** 本机多个 profile 的 cron 并发运行，其中 **demo-tester 的定时任务会执行 `rm -f /tmp/gh_payload_*.json`**（其自身技能笔记记载回收「来自兄弟会话的 200+ 个文件」）。后果不止覆盖，而是**把正在运行的其他进程的中间文件直接删掉**：当日 demo-pm 备份脚本上传到第 3 个（最大约 100KB）blob 时失败 —— `FATAL uploading ...: open /tmp/gh_payload_....json: no such file or directory`。**教训：不要把共享 /tmp 的通用前缀当成自己的**。需要跨进程存活的中间文件一律用 `tempfile.mkdtemp(prefix="<独特前缀>-")`（私有目录 + 不匹配兄弟进程 glob 的前缀），别用 `/tmp/<通用名>_<时间戳>.json`。诊断这类「文件凭空消失」时，用 `search_files` 在整个 `~/.hermes` 树里 grep 那个命名空间（技能目录 + `logs/`），确认谁还会碰它 —— 本轮正是这样定位到 demo-tester 的。
