...
Dated run notes: `references/demo-pm-backup-workflow-YYYYMMDD.md` (latest: 2026-08-30; full list in Reference Files).
...
- `references/demo-pm-backup-workflow-20260828.md` — 2-commit Method B run: 7 new root health diag scripts read .env; `pm_health`/`gh_health` broadening; ⚠️ blanket `health_` near-miss would delete legit nested comfyui health_check.py → root-scoped check (pitfall above)
- `references/demo-pm-backup-workflow-20260829.md` — clean Method B run (subtree script, REPO_OWNER=OnePlusNDev): commit b1724919 (5M+1A); config.yaml all 15 api_key empty → no key_env replacement; new `triage_verify.py` exclude gap (root-level .env reader, name matched NO pattern — added to EXCLUDE_NAMES in both scripts + SKILL.md lists); preflight b64 doc-placeholder false positive fixed (`real()` decodes b64 and filters placeholder chars `*`/`xxx`/`...`); stale local clone (ed4bcb8 vs remote c928b535) → Method B not A — rule: local clone present ≠ Method A, verify local HEAD == remote HEAD first
- `references/demo-pm-backup-workflow-20260830.md` — clean Method B run: main commit daab0134 (4M+3A: cron/jobs.json, memories/archive/ARCHIVE.md, backup-skill SKILL.md + preflight script, 3 new skill files) + follow-up commit 2bb2b656 (dated run note written AFTER the main commit lands as a separate commit and appears as `A` in the NEXT run's preflight — expected, not an exclude gap); config.yaml all 15 api_key empty → no key_env replacement; preflight CLEAN (no exclude gaps, no redaction needed); post-push verification: 0 plaintext keys, no sensitive/temp files in remote tree, demo-pm 590 blobs, siblings intact (total 609)
