...
Dated run notes: `references/demo-pm-backup-workflow-YYYYMMDD.md` (latest: 2026-08-31; full list in Reference Files).
...
- `references/demo-pm-backup-workflow-20260829.md` — clean Method B run: commit b1724919 (5M+1A); config.yaml all 15 api_key empty → no key_env; new `triage_verify.py` exclude gap (root-level .env reader, name matched NO pattern → EXCLUDE_NAMES in both scripts + SKILL.md lists); preflight b64 doc-placeholder false positive fixed (`real()` decodes b64, filters placeholder chars); stale local clone → Method B — rule: local clone present ≠ Method A, verify local HEAD == remote HEAD first
- `references/demo-pm-backup-workflow-20260830.md` — clean Method B run: main daab0134 (4M+3A) + follow-up 2bb2b656 (run note lands as separate commit, shows as `A` next run — expected); config.yaml 15 api_key empty → no key_env; preflight CLEAN; post-push: 0 plaintext keys, no sensitive files, demo-pm 590 blobs, siblings intact (609)
- `references/demo-pm-backup-workflow-20260831.md` — clean Method B run: main a1a6cd1a (5M+1A) + follow-up ea4f18cd (run note); **cron started with OnePlusNTester active (read-only on target, `push=false` — third account vs owner OnePlusNDev / collaborator OnePlusNPM) → switch + `.permissions.push` confirm before Method B; GITHUB_TOKEN unset; JungleAssistant in keyring**; clone stale (ed4bcb8) → Method B; preflight CLEAN; post-push: 0 plaintext keys, no sensitive files, demo-pm 591 blobs, siblings intact (610)
