...
- `**/pm_healthcheck_*.py` — dated one-off health-check scripts (e.g. `pm_healthcheck_0807b.py` discovered 2026-08-08; reads GITHUB_TOKEN from .env — diagnostic artifact, not config)
...
This is the reliable form of the tree-leak check; single-path exact checks (`select(.path=="demo-pm/...")`) still work fine in `--jq`.
