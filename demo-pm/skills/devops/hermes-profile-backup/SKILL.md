...
Dated run notes: `references/demo-pm-backup-workflow-YYYYMMDD.md` (latest: 2026-09-25; full per-run index in `references/dated-runs-index.md`).
...
- `skills/.archive/` — curator archive of stale bundled skills; see `references/curator-archive-churn-triage.md`
...
- `gh_health*`, `healthcheck_*.py`, root `health_*` — root-level diagnostics that read `.env` (same family as `get_token.sh`); covered by the `pm_health`/`gh_health`/`healthcheck_` prefixes plus a ROOT-scoped `health_` check (scripts use `len(parts)==1`, so nested legit files such as `skills/creative/comfyui/scripts/health_check.py` survive — verified 2026-08-28)
...
- `**/skills/.curator_suppressed` — curator suppression list (runtime state)
