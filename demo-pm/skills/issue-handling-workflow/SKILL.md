...
> ⚠️ **CRITICAL: Skip `.env` when using `gh`.** If `gh auth status` passes (and it will if the profile was set up correctly), you do NOT need to `source .env`, `cat .env`, or `read_file .env`. `gh` stores its own auth token at `~/.config/gh/hosts.yml` and uses it transparently. Every call to `source /path/to/.env` or `read_file .env` (which will be blocked by credential protection) is wasted work when `gh` is available. **Verify once with `gh auth status`, then use `gh` commands directly.** Only source `.env` if you're forced to fall back to `curl` + `$GITHUB_TOKEN`.
